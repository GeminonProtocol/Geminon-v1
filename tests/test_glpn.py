...
deploy_args = {'V3Aggregator': [8, 1800*1e8],  # uint8 _decimals, int256 _initialAnswer
               ...
               'GenLiqPoolNat': [None, # gexToken address of the GEX token contract
                                 500, # poolWeight_ integer percentage 3 decimals [1, 1000] (1e3)
                                 0.01 * 1e18, # initPoolPrice_ must be in 1e18 USD units
                                ],
...
def test_setSCMinter(contract_glp, contract_scminter):
    ...
    with reverts():
        contract_glp.setSCMinter(accounts[2], {"from": accounts[0]})
...
def test_setPoolWeight(contract_glp, contract_glp2, contract_token2, contract_oracle):
    ...
    contract_glp.mintSwapNative(0, {"from": arb_addr, "value": 1*1e18})
...
def test_mintRedeem_alt(contract_glp, contract_gex, contract_oracle):
    ...
    data = []
    data.append((contract_glp.blockTimestampLast(), contract_glp.lastPrice()))
    ...
    for i in range(20):
        ...
        pd.DataFrame(data, columns=['timestamp','meanMintRatio'])\
            .to_csv('d:/Proyectos/Geminon/python/data/test_glp_mint_burn_alt.csv', 
                    index=False)
...
#             # print(f'Gex amount: {amount/1e18}, balance collat pool: {contract_glp.balanceCollateral()/1e18}')
#             # print(f'Minted GEX pool: {contract_glp.mintedGEX()/1e18}, balance GEX pool: {contract_glp.balanceGEX()/1e18}')
#             # fee = contract_glp.amountRedeemFee(amount)
#             # print(f'Fee: {fee/1e18}')
#             # print(contract_oracle.getTotalCollatValue()/1e18)
#             # print(contract_glp.getCollateralValue()/1e18)
#             # print(contract_glp._poolWeightRatio())
#             # print(contract_glp._burnRatio())
#             # amount_burned = contract_glp._burnAmount(amount)
#             # print(f'Burned amount: {amount_burned/1e18}')
#             # amount_out = contract_glp.amountOutCollateral(amount - fee)
#             # print(f'Out collateral amount: {amount_out/1e18}, balance collat pool: {contract_glp.balanceCollateral()/1e18}')
...
#         pd.DataFrame(data, columns=['timestamp','meanMintRatio'])\
#             .to_csv('d:/Proyectos/Geminon/python/data/test_glp_mint_burn_rand.csv', 
#                     index=False)
...
#         pd.DataFrame(data, columns=['timestamp','meanMintRatio'])\
#             .to_csv('d:/Proyectos/Geminon/python/data/test_glp_multiPool_rand.csv', 
#                     index=False)
...
def test_lendCollateral(contract_glp, contract_scminter, 
...
# def test_pre_updateOracle(contract_glp, contract_oracle):
...
def with_reduce_pool_weight(target_weight, contract_glp, contract_glp2, contract_oracle): 
    ...
    while actual_weight > target_weight:
        ...
        local_target_weight = max(contract_glp.poolWeight() - 50, target_weight)
        ...
        while actual_weight > local_target_weight:
            ...
            chain.mine()
            tx = contract_glp2.mintSwap(int(1e17), 0, {"from": accounts[0]})
...
def with_reduce_pool_native_weight(target_weight, contract_glp, contract_glp2, contract_oracle): 
    ...
    while actual_weight > target_weight:
        ...
        while actual_weight > local_target_weight:
            ...
            chain.sleep(2*i*3600)
            chain.mine()
            tx = contract_glp2.mintSwapNative(0, {"from": accounts[0], "value": int(1e17)})
            contract_glp.redeemSwap(tx.return_value, 0, {"from": accounts[0]})
            actual_weight = contract_oracle.getPoolCollatWeight(contract_glp.address)
